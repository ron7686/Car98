...
@WebServlet("/_00_init/getBidImage")
public class RetrieveBidImageServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			BidBean bean = bidService.getBid(bidId);
			if (bean != null) {
				blob = bean.getBidPic();
				if (blob != null) {
...
//				fileName = bean.getFileName();
			...
			ex.printStackTrace();
			throw new RuntimeException("_00_init.util.RetrieveBidImageServlet#doGet()發生SQLException: " + ex.getMessage());
		} finally{
			if (is != null) is.close();
			if (os != null) os.close();
